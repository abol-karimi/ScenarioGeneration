...
def plot_curve(gen_config, test_config, plot_color, plot_label, axes):
  ...
  # if gen_config['SUT-config']['ego-module'] != test_config['SUT-config']['ego-module']: # surrogate
  #   offset = int(gen_config['max-total-time']/60)
  #   elapsed_times = tuple(t+offset for t in elapsed_times)
...
def plot(plot_configs):
  ...
  fig_coverage.savefig(f'experiments/ISSTA/target-vs-surrogate_{test_coverage}_violations_per-time.png')