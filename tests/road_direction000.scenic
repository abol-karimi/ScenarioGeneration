...
for d in np.arange(10, lane.centerline.length, 10):
...
monitor ShowRotations():
  # for car in simulation().agents:
  #   print(car.orientation)
 
  for angle in range(-180, 180, 1):
    visualization.draw_transform(simulation().world, carla.Location(angle*.1, 0, 10), carla.Rotation(roll=angle))
    visualization.draw_transform(simulation().world, carla.Location(0, angle*.1, 10), carla.Rotation(pitch=angle))
    visualization.draw_transform(simulation().world, carla.Location(0, 0, angle*.1+10), carla.Rotation(yaw=angle))
  
  wait

require monitor ShowRotations()
